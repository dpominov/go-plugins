...
func (g *grpcClient) Call(ctx context.Context, req client.Request, rsp interface{}, opts ...client.CallOption) error {
	...
	log.Log(req.Service(),"g.next")
	next, err := g.next(req, callOpts)
	if err != nil {
		return err
	}
	log.Log(req.Service(),"deadline")
	...
	log.Log(req.Service(),"wait done")
	// should we noop right here?
	select {
	case <-ctx.Done():
		log.Log(req.Service(),"ctx->Done")
	...
	gcall := g.call
	log.Log(req.Service(),"wrap")
	// wrap the call in reverse
	for i := len(callOpts.CallWrappers); i > 0; i-- {
		gcall = callOpts.CallWrappers[i-1](gcall)
	}
	log.Log(req.Service(),"declare call")
	// return errors.New("go.micro.client", "request timeout", 408)
	call := func(i int) error {
		// call backoff first. Someone may want an initial start delay
		log.Log(req.Service(),"backoff",i)
		t, err := callOpts.Backoff(ctx, req, i)
		if err != nil {
			log.Log(req.Service(),"backoff error", err)
			return errors.InternalServerError("go.micro.client", err.Error())
		}

		log.Log(req.Service(),"before sleep",i)
		// only sleep if greater than 0
		if t.Seconds() > 0 {
			time.Sleep(t)
		}
		log.Log(req.Service(),"before select next node",i)
		// select next node
		node, err := next()
		log.Log(req.Service(),"next node",i,"err",err)
		if err != nil && err == selector.ErrNotFound {
			return errors.NotFound("go.micro.client", err.Error())
		} else if err != nil {
			return errors.InternalServerError("go.micro.client", err.Error())
		}
		log.Log(req.Service(),"next ",i,"node ",node.Id,node.Address,node.Port)
		...
		// make the call
		log.Log(req.Service(),"make call ",i)
		err = gcall(ctx, addr, req, rsp, callOpts)
		log.Log(req.Service(),"call",i," err",err)
	...
	for i := 0; i <= callOpts.Retries; i++ {
		go func() {
			...
			ch <- call(i)
			log.Log(req.Service(),"after call",i)
		}()

		select {
		case <-ctx.Done():
			log.Log(req.Service(),"done",i, "ctx.Err()",ctx.Err())
			return errors.New("go.micro.client", fmt.Sprintf("%v", ctx.Err()), 408)
		case err := <-ch:
			log.Log(req.Service(),"err",err)
			// if the call succeeded lets bail early
			if err == nil {
				return nil
			}

			log.Log(req.Service(),"before callOptions ",i)
			retry, rerr := callOpts.Retry(ctx, req, i, err)
			log.Log(req.Service(),"rerr",i, rerr)
			if rerr != nil {
				return rerr
			}

			log.Log(req.Service(),"retry",i, retry)
			if !retry {
				return err
			}
